...
int main (int argc, char ** argv)
{
	...
	int * array1 = (int*)sds_alloc(sizeof(int)*N); // 1. Change the way shared data are allocated
	assert(array1!=NULL);

	int * array2 = (int*)sds_alloc(sizeof(int)*N);
	assert(array2!=NULL);
	
	srand(0); // 1. To get reproducible results
	...
	for(i=0;i<N;i++)
	{
		...
		// 2. To check against the waveform
		if(i<10)
		{
	...
	int * array3_hw = (int*)sds_alloc(sizeof(int)*N);
	...
	for(i=0;i<N;i++)
	{
		assert(array3_hw[i]==T+i); // 3. To check in the waveform
	}
