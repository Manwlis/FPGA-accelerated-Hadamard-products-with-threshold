...
int main(int argc, char ** argv)
{
	...
	// fixed gia accelerator
	dataType_fixed * data0_fixed = (dataType_fixed *)malloc(sizeof(dataType_fixed)*dim*dim);
	assert(data0_fixed!=NULL);
	...
	for(i=0;i<dim*dim;i++)
	{
		...
		data0[i] = t+d;
		data0_fixed[i] = (dataType_fixed) (t+d);
		//printf("%d: %f %f\n", i, data0[i] , (float) data0_fixed[i]);
	}
	...
	// fixed gia accelerator
	dataType_fixed * data1_fixed = (dataType_fixed *)malloc(sizeof(dataType_fixed)*dim*size);
	assert(data1_fixed!=NULL);
	...
	for(i=0;i<dim*size;i++)
	{
		...
		dataType_t d = ((float)(rand()%10))/10;
		data1[i] = t+d;
		data1_fixed[i] = (dataType_fixed) data1[i];
	}
	...
	dataType_fixed_l * data2_hw = (dataType_fixed_l *)malloc(sizeof(dataType_fixed_l)*dim*size);
	...
	// klhsh accelerators
	myFuncAccel4(size, dim, (dataType_fixed_l) threshold, data0_fixed, data1_fixed, data2_hw);
	...
	for(i = 0; i < size; i++){
		for(j = 0; j < dim; j++){
			...
			sprintf(string_hw, "%.2f", data2_hw[i*dim + j].to_float());
