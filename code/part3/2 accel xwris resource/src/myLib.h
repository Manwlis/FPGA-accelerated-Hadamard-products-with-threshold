...
void myFuncAccel4_1 (unsigned int size, unsigned int dim, dataType_t threshold, dataType_t data0[16], dataType_bus * data1, dataType_bus * data_out);

#pragma SDS data mem_attribute(data0:PHYSICAL_CONTIGUOUS, data1:PHYSICAL_CONTIGUOUS, data_out:PHYSICAL_CONTIGUOUS)
#pragma SDS data zero_copy(data0[0:dim*dim], data1[0:size], data_out[0:size])
#pragma SDS data access_pattern(data0:SEQUENTIAL, data1:SEQUENTIAL, data_out:SEQUENTIAL)
void myFuncAccel4_2 (unsigned int size, unsigned int dim, dataType_t threshold, dataType_t data0[16], dataType_bus * data1, dataType_bus * data_out);
